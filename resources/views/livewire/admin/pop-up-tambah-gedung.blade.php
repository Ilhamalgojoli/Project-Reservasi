<section
    class="popup fixed inset-0 bg-gray-900/80 flex items-center justify-center z-[100] p-4 transition-all duration-500">
    <div
        ...
        <div class="p-8 flex items-center justify-between bg-white border-b border-gray-100 shrink-0">
            <div class="flex items-center gap-4 text-gray-800">
                ...
                <div>
                    <h2 class="text-2xl font-black tracking-tight leading-none">Tambah Data Gedung
                    </h2>
                    <p class="text-[10px] font-medium text-gray-400 uppercase tracking-[0.2em] mt-2">Daftarkan Gedung baru</p>
                </div>
            </div>
            <button type="button" wire:click="closeButton()"
                class="w-12 h-12 shrink-0 flex items-center justify-center rounded-[8px] bg-gray-50 text-gray-400 border border-gray-100 hover:bg-red-50 hover:text-red-500 transition-all duration-300">
                <iconify-icon icon="solar:close-circle-bold" class="text-2xl"></iconify-icon>
            </button>
        </div>

        <form id="tambah-gedung" wire:submit.prevent="submit" class="flex flex-col flex-1 overflow-hidden">
            <div class="p-8 flex flex-col gap-8 flex-1 overflow-y-auto custom-scrollbar">
                ...
                <div class="flex flex-col gap-6">
                    ...
                    <div class="flex flex-col gap-5">
                        <div class="relative group">
                            <label
                                class="text-[11px] font-medium uppercase tracking-widest text-gray-400 ml-1 mb-2 block">Foto
                                / Ilustrasi Gedung</label>
                            <div class="relative">
                                <iconify-icon icon="solar:gallery-add-bold"
                        ...
                        <div class="flex flex-row sm:flex-col gap-5">
                            <div class="flex-1 space-y-2">
                                ...
                                @error('nama')
                                    <span
                                        class="text-[10px] text-red-500 font-bold uppercase ml-1">{{ $message }}</span>
                                @enderror
                            </div>
                            <div class="flex-1 space-y-2">
                                <label class="text-[11px] font-medium uppercase tracking-widest text-gray-400 ml-1">Kode
                                    / Singkatan</label>
                                <div class="relative group">
                        ...
                        <div class="flex flex-row sm:flex-col gap-5">
                            <div class="flex-1 space-y-2">
                                <label
                                    class="text-[11px] font-medium uppercase tracking-widest text-gray-400 ml-1">Jumlah
                                    Lantai</label>
                                <div class="relative group">
                                    <iconify-icon icon="solar:layers-bold-duotone"
                                        class="absolute left-4 top-1/2 -translate-y-1/2 text-gray-400 group-focus-within:text-[#e51411] transition-colors text-lg"></iconify-icon>
                                    <input type="number" wire:model="jumlahLantai"
                                        class="w-full pl-11 pr-4 py-3 bg-gray-50 border border-gray-200 rounded-xl text-sm font-medium text-gray-800 focus:bg-white focus:ring-4 focus:ring-[#e51411]/5 focus:border-[#e51411] transition-all outline-none"
                            ...
                            <div class="flex-1 space-y-2">
                                <label
                                    class="text-[11px] font-medium uppercase tracking-widest text-gray-400 ml-1">Status
                                    Operasional</label>
                                <div class="relative group">
                        ...
                        <div class="space-y-2">
                            <label
                                class="text-[11px] font-medium uppercase tracking-widest text-gray-400 ml-1">Deskripsi
                                Singkat</label>
                            <div class="relative group">
                                <textarea wire:model="keterangan"
                                    class="w-full h-24 p-4 bg-gray-50 border border-gray-200 rounded-xl text-sm font-medium text-gray-800 focus:bg-white focus:ring-4 focus:ring-[#e51411]/5 focus:border-[#e51411] transition-all outline-none resize-none"
                                    placeholder="Jelaskan fungsi atau lokasi gedung..."></textarea>
                            </div>
                            @error('keterangan')
                                <span class="text-[10px] text-red-500 font-bold uppercase ml-1">{{ $message }}</span>
                            @enderror
                        </div>
                    </div>
                </div>
                ...
                <div class="flex flex-col gap-6">
                    ...
                    <div class="space-y-4">
                        <div class="rounded-[8px] overflow-hidden border border-gray-100 shadow-sm" wire:ignore>
                            <div id="map" class="h-[250px] w-full bg-gray-100"></div>
                        </div>

                        <div
                            class="flex items-center justify-between px-5 py-3 bg-gray-50 rounded-[8px] border border-gray-100">
                            <div class="flex gap-6">
                                <div class="flex items-center gap-2">
                                    <span
                                        class="text-[9px] font-black text-gray-400 uppercase tracking-widest">Lat:</span>
                                    <span id="show-lat" class="text-xs font-bold text-gray-800 italic">0.000</span>
                                </div>
                                <div class="flex items-center gap-2">
                                    <span
                                        class="text-[9px] font-black text-gray-400 uppercase tracking-widest">Lng:</span>
                        ...
                        @error('latitude')
                            <span
                                class="text-[10px] text-red-500 font-bold uppercase ml-1 block">{{ $message }}</span>
                        @enderror
            ...
            <div class="p-8 bg-gray-50/50 border-t border-gray-100 shrink-0">
                <button type="submit"
                    class="w-full flex items-center justify-center gap-3 px-6 py-3.5 sm:px-12 sm:py-5 bg-[#e51411] text-white rounded-[8px] font-black text-xs uppercase tracking-[0.25em] hover:bg-red-700 hover:-translate-y-1 transition-all shadow-xl">
